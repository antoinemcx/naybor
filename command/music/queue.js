module.exports={
    ...
    run: async (bot, message, args) => {
        ...
        message.reply({embeds: [{
            ...
            author: { name: `${bot.language.QUEUE[0]} ${message.guild.name}${queue.loopMode ? ' (looped)' : ''}`, icon_url: message.author.displayAvatarURL({dynamic: true}) },
            description: `${bot.language.QUEUE[1]} : [${queue.current.title}](${queue.current.url}) » \`${bot.language.QUEUE[2]} : ${queue.current.requestedBy.username}\`\n\n${list}`,
